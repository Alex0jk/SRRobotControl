...
class FirebaseRpiController(robotName: String) {

    private var TAG:String = "GPS Firebase"
    private var dbReference:DatabaseReference

    init{
        this.dbReference = FirebaseDatabase.getInstance().reference.child(robotName)
    }
    ...
    private val mLocationListener = object : LocationListener {
        override fun onLocationChanged(location: Location) {
            try {
                val msg = "${String.format("%.5f", location.latitude)};${String.format("%.5f",location.longitude)};speed:${location.speed}"
                Log.v(TAG, msg)
